...
sCurrentVersionScript="v6"
iTime1=time.time()
########################################################################
'''
V6-2022/03/18
Hack to insert sSID in name for 1by1 assembly
...
python MappingReverseMegahit.py -i INPUT -p PID -m MULTIPLEX -s SAMPLEID
...
parser.add_option("-s","--sampleid", dest="sampleid")
...
sSampleId=options.sampleid
if not sSampleId:
	exit("Error : no sampleid -s defined, process broken")
...
########################################################################
#Function 	
def WriteContigFiles(dContig2Read,bMultiplex,sSID):
		...
		if sNewLine[0]==">":
			if sId!="" and sContent!="":
				if bRejected:
					...
					iNameId+=1
					sNewName=CONTIG_BASENAME+"_"+str(iNameId)+"_"+str(sSID)+"_("+str(len(dContig2Read[sShortId][READS]))+")"
					if bMultiplex:
...
if __name__ == "__main__":
	...
	WriteContigFiles(dContig2Read,bMultiplex,sSampleId)
