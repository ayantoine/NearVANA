...
if [ "$USE_MULTIPLEX" = true ] ; then
	...
	if [ "$USE_PREFILTER" = true ] ; then
		...
		if [ "$USE_DIAMOND" = true ] ; then
			echo "Using Diamond"
			if [ ! -f ${PID}.StatBlastD.ok ]; then
				echo "python ${SDIR}/Extract4Stat_all.py -i ${PID}_BlastD_results.tab -o ${PID}_Stat_BlastD/ -v ${VMR} -r ${PID}_All.Megahit_reverseAssembly.tsv -d ${DATA} -1 ${ALLFAM} -2 ${ALLGEN} -3 ${ALLSPE} > Stat_BlastD.o"
				python ${SDIR}/Extract4Stat_all.py -i ${PID}_BlastD_results.tab -o ${PID}_Stat_BlastD/ -v ${VMR} -r ${PID}_All.Megahit_reverseAssembly.tsv -d ${DATA} -1 ${ALLFAM} -2 ${ALLGEN} -3 ${ALLSPE} > Stat_BlastD.o
				touch ${PID}.StatBlastD.ok
			else
				echo "${PID}.StatBlastD.ok already existing, pass"
			fi
		...
		if [ "$USE_BLASTX" = true ] ; then
			echo "Using BlastX"
			if [ ! -f ${PID}.StatBlastX.ok ]; then
				echo "python ${SDIR}/Extract4Stat_all.py -i ${PID}_BlastX_results.tab -o ${PID}_Stat_BlastX/ -v ${VMR} -r ${PID}_All.Megahit_reverseAssembly.tsv -d ${DATA} -1 ${ALLFAM} -2 ${ALLGEN} -3 ${ALLSPE} > Stat_BlastX.o"
				python ${SDIR}/Extract4Stat_all.py -i ${PID}_BlastX_results.tab -o ${PID}_Stat_BlastX/ -v ${VMR} -r ${PID}_All.Megahit_reverseAssembly.tsv -d ${DATA} -1 ${ALLFAM} -2 ${ALLGEN} -3 ${ALLSPE} > Stat_BlastX.o
				touch ${PID}.StatBlastX.ok
			else
				echo "${PID}.StatBlastX.ok already existing, pass"
			fi
		...
		if [ "$USE_BLASTN" = true ] ; then
			echo "Using BlastN"
			if [ ! -f ${PID}.StatBlastN.ok ]; then
				echo "python ${SDIR}/Extract4Stat_all.py -i ${PID}_BlastN_results.tab -o ${PID}_Stat_BlastN/ -v ${VMR} -r ${PID}_All.Megahit_reverseAssembly.tsv -d ${DATA} -1 ${ALLFAM} -2 ${ALLGEN} -3 ${ALLSPE} > Stat_BlastN.o"
				python ${SDIR}/Extract4Stat_all.py -i ${PID}_BlastN_results.tab -o ${PID}_Stat_BlastN/ -v ${VMR} -r ${PID}_All.Megahit_reverseAssembly.tsv -d ${DATA} -1 ${ALLFAM} -2 ${ALLGEN} -3 ${ALLSPE} > Stat_BlastN.o
				touch ${PID}.StatBlastN.ok
			else
				echo "${PID}.StatBlastN.ok already existing, pass"
			fi
