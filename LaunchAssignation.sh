...
if [ "$USE_PAIREND" = true ] ; then
	...
	python ${SCRIPTDIR}/MakeAssignation.py -p 1 -v ${VARNAME} -q ${SEQ_NUMBER} -1 ${PID}_${VARNAME}_R1.fastq -2 ${PID}_${VARNAME}_R2.fastq -k ${KMER_FILE} -d ${WORKDIR} -t ${OKDIR}/${DIGITID}_MakeAssignation.ok -i ${DIGITID} -c ${CONFFILE}
else
	...
	python ${SCRIPTDIR}/MakeAssignation.py -p 0 -v ${VARNAME} -q ${SEQ_NUMBER} -1 ${PID}_${VARNAME}_R1.fastq -k ${KMER_FILE} -d ${WORKDIR} -t ${OKDIR}/${DIGITID}_MakeAssignation.ok -i ${DIGITID} -c ${CONFFILE}
fi
