...
echo "----------------------------------------"
echo cutadapt -a $adapter1 -A $adapter2 -q 30 -O $((${#adapter1}*85/100)) -m 15 -j 0 -o ${PID}_Cleaning/${DIGITID}_R1.Trim1.fastq -p ${PID}_Cleaning/${DIGITID}_R2.Trim1.fastq ${PID}_Cleaning/1Clean_${DIGITID} ${PID}_Cleaning/2Clean_${DIGITID}
