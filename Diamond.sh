...
if [ ! -f Blast${TASK}_Ok/${STASKID}_Blast${TASK}.ok ]; then
    ...
    diamond blastx -d ${VIRPTDB} -q ${PID}"_ToDiamond"/${PID}_All.fa.${STASKID} -o ${PID}"_Diamond"/${PID}_All.fa.${STASKID}.Diamond_1.tab -p ${MULTICPU} --max-target-seqs 1 --max-hsp 1 --evalue 0.001 --block-size $((${MULTIMEMORY}/6))
    ...
    diamond blastx -d ${ALLPTDB} -q ${PID}"_Diamond"/${PID}_All.fa.${STASKID}.keeped -o ${PID}"_Diamond"/${PID}_All.fa.${STASKID}.Diamond_2.tab -p ${MULTICPU} --max-target-seqs 5 --max-hsp 1 --evalue 0.001 --block-size $((${MULTIMEMORY}/6))
    echo "------ /Complete database blast ------"

    touch Diamond_Ok/Diamond${TASK}.ok
else
